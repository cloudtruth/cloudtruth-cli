...
/// Use the token to wrap the plaintext bytes in the specified algorithm
fn secret_wrap(
...
pub fn secret_encode_wrap(
    ...
    secret_wrap(algorithm, token, encoded.as_bytes())
}

/// Uses the token to unwrap the encrypted bytes
fn secret_unwrap(token: &[u8], encoded: &str) -> Result<Vec<u8>, CryptoError> {
...
mod test {
    ...
    #[test]
    fn chacha20_coded_wrap_and_unwrap() {
        let wrapped =
            secret_encode_wrap(CryptoAlgorithm::ChaCha20, TEST_JWT.as_bytes(), TEST_SECRET)
                .unwrap();
        let algo_name = CryptoAlgorithm::ChaCha20.to_string();
        assert!(wrapped.contains(&algo_name));
        let unwrapped = secret_unwrap_decode(TEST_JWT.as_bytes(), &wrapped).unwrap();
        assert_eq!(TEST_SECRET, unwrapped);
    }
    ...
    #[test]
    fn aes_gcm_coded_wrap_and_unwrap() {
        let wrapped =
            secret_encode_wrap(CryptoAlgorithm::AesGcm, TEST_JWT.as_bytes(), TEST_SECRET).unwrap();
        let algo_name = CryptoAlgorithm::AesGcm.to_string();
        assert!(wrapped.contains(&algo_name));
        let unwrapped = secret_unwrap_decode(TEST_JWT.as_bytes(), &wrapped).unwrap();
        assert_eq!(TEST_SECRET, unwrapped);
