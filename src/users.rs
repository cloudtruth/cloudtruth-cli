use crate::cli::{
    CONFIRM_FLAG, DELETE_SUBCMD, DESCRIPTION_OPT, FORMAT_OPT, LIST_SUBCMD, NAME_ARG, SET_SUBCMD,
...
fn proc_users_list(
    ...
    if details.is_empty() {
        ...
        let mut hdr = vec!["Name", "Type", "Email", "Description"];
        let mut properties = vec!["name", "type", "email", "description"];
        if show_times {
            hdr.push("Created At");
