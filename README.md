...
The Sigma Config CLI tool is used for interacting with the [Sigma Config configuration management service](https://sigma-automate.com).

In order to use this utility you must have an active Sigma Config account and generate a personal access token for authenticating with the API.

- [Sigma Config CLI](#sigma-config-cli)
...
The Sigma Config CLI tool has several ways of configuring it, allowing you to pick whichever method is most suitable for your environment.
...
The Sigma Config CLI utility stores its configuration in the YAML format. The configuration file must be named _cli.yml_ and must reside in the standard application configuration location for your platform:
...
The Sigma Config CLI utility maps all environment variables with the `CLOUDTRUTH_` followed by a configuration name to the same settings as would be available in the configuration file format.
...
* `$CLOUDTRUTH_PROJECT` -> Your "default" Sigma Config project
* `$CLOUDTRUTH_ENVIRONMENT` -> Your "default" Sigma Config environment


### Argument-based Configuration

The Sigma Config CLI utility is able to be configured by supplying command-line arguments for the necessary settings. As with the other configuration options available to you, care should be taken to ensure secret values are not exposed to other users on your system.
...
Once you have the application configured with your Sigma Config API key, you can interact with your Sigma Config data by running commands.

The Sigma Config CLI application uses subcommands to scope available actions as appropriate for a given resource. You can see the full list of commands by running:
...
Sigma Config CLI profiles are a way of organizing the application's configuration data into multiple named groups. Profiles, in this sense, are unrelated to configuration values in your Sigma Config account. They simply allow you to configure the Sigma Config CLI for multiple organizations or multiple API keys with different access restrictions.
...
### Switching Active Sigma Config Environment

By default, all commands will run against the _default_ Sigma Config environment.
