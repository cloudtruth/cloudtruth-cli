...
/// struct for typed errors of method [`serviceaccounts_rekey_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ServiceaccountsRekeyCreateError {
    UnknownValue(serde_json::Value),
}
...
pub fn serviceaccounts_rekey_create(
    configuration: &configuration::Configuration,
    id: &str,
    service_account_rekey_request: Option<crate::models::ServiceAccountRekeyRequest>,
) -> Result<crate::models::ServiceAccountRekeyRequest, Error<ServiceaccountsRekeyCreateError>> {
    let local_var_configuration = configuration;
    ...
    local_var_req_builder = local_var_req_builder.json(&service_account_rekey_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ServiceaccountsRekeyCreateError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
...
///              Updates a ServiceAccount.  A ServiceAccount is a user record intended             for machine use (such as a build system).  It does not have a username/password             but is instead accessed using an API key.             
